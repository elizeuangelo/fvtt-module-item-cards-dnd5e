...
function addTab(sheet, html, data) {
	...
	// Listeners
	activateTabListeners(menu, sheet, tab);
	activateTabContentListeners(sheet, tab);
}

function activateTabListeners(menu, sheet, tab) {
	menu.on('click', () => setTimeout(() => sheet.setPosition({ height: 'auto' }), 0));
	tab.find('input,select,textarea').on('change', () => Hooks.once('renderItemSheet', () => sheet.element.find('nav [data-tab=item-card]')[0].click()));
}

export function activateTabContentListeners(sheet, tab) {
